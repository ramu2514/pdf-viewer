...
public class ReaderView extends AdapterView<Adapter>
        implements GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener, GestureDetector.OnDoubleTapListener, Runnable {
    ...
    private final boolean mLinksEnabled = true;
    ...
    public ReaderView(Context context, boolean pdf) {
        super(context);
        setup(context);
        isPDF = pdf;
    }
    ...
    public void enableSelectMode(MotionEvent e) {
        if (!isPDF) return;
        mMode = Mode.Selecting;
        onSelectModeChange(true);
        PageView pageView = (PageView) getDisplayedView();
        if (pageView == null) return;
        pageView.selectText(e);
        pageView.setOnSelectionChangeListener(this::dismissPopup);
    }
    ...
    private void setup(Context context) {
        ...
        HORIZONTAL_SCROLLING = sp.getBoolean(AppConstants.READER_HORIZONTAL_SWIPE, false);
    ...
    public void goToNextPage() {
        Log.e("Page ", String.valueOf(mCurrent));
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
                           float velocityY) {
        if (mScaling || mMode == Mode.Selecting)
            return true;
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        Log.d("DebugTouch", "onSingleTapUp - Touch disabled :" + tapDisabled);
        if (mMode == Mode.Selecting) {
            return true;
        }
        if (!tapDisabled) {
            ...
            List<PDFWidget> widgets = new ArrayList<>();
            if (pageView != null) {
                annotations = pageView.getAnnotationsAtLocation(e.getX(), e.getY());
                widgets = pageView.getPDFWidgets(e.getX(), e.getY());
            }
            ...
            onTapMainDocArea(annotations, widgets);
        }
        return true;
    }
    ...
    public void dismissPopup() {
        ((PageView) getDisplayedView()).setSearchBoxes(null);
        onSelectModeChange(false);
        mMode = Mode.Viewing;
    }

    public String getVisibleText() {
        return ((PageView) getDisplayedView()).getVisibleText();
    }

    enum Mode {Viewing, Selecting}
